...
export default function AdminDashboard(){
  ...
  async function genQR(){
    try {
      ...
      if (er) throw er;
      const units = (r||[]).map(x=>x.unit_id).filter(Boolean);
      if (!units.length){ alert('No residents found. Upload residents first.'); return; }

      setProgress(10); setProgressMsg('Creating tokens…');
      const expires = new Date(Date.now() + 14*24*3600*1000).toISOString();
      const batch = units.map(u=>({ token: crypto.randomUUID(), unit_id: u, expires_at: expires }));
      const { error: eu } = await supabase.from('qr_tokens').upsert(batch, { onConflict: 'token' });
      if (eu) throw eu;

      setProgress(40); setProgressMsg('Preparing QR PDF…');
      const pdf = new jsPDF({ unit: 'pt', format: 'a4' });
      const margin = 36;
      const cellW = 250, cellH = 260;
      const cols = 2;
      let x = margin, y = margin;
      const total = batch.length;
      let done = 0;

      const prefix = import.meta.env.VITE_QR_URL_PREFIX || (window.location.origin + window.location.pathname + '#/');
      for (const row of batch){
        // URL can include unit and token if you want to validate later
        const url = `${prefix}?unit_id=${encodeURIComponent(row.unit_id)}&token=${encodeURIComponent(row.token)}`;
        const dataUrl = await QRCode.toDataURL(url, { width: 220, margin: 1 });

        pdf.setFontSize(12);
        pdf.text(`Unit: ${row.unit_id}`, x, y);
        pdf.addImage(dataUrl, 'PNG', x, y+8, 200, 200);
        pdf.text(url, x, y+220, { maxWidth: cellW-10 });
      ...
      await supabase.from('vote_audit').insert({ action:'qr_generate', details:{ count: batch.length, expires_at: expires, pdf:true } });
      setProgressMsg('Done');
    } catch(e){
...
function exportPDF(){ window.print(); }
  ...
  async function resetResidents(){
    ...
    const { error } = await supabase.from('residents').delete().not('unit_id','is',null);
    if (error){ alert('Reset residents failed: '+error.message); return; }
    await supabase.from('vote_audit').insert({ action:'residents_reset' });
    alert('Residents cleared.');
    location.reload();
  }
