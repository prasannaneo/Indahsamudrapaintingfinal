...
function PublicVote(){
  ...
  async function confirmVote(){
    ...
    if(error){ alert("Vote failed: " + error.message); setConfirmId(null); return; }
...
function AdminDashboard(){
  ...
  // Fetch-fresh exports (not relying on current state)
  async function exportTallies(){
    const { data:v } = await supabase.from("votes").select("option_id").eq("poll_id",pollId);
    const t={A:0,B:0,C:0}; (v||[]).forEach(r=>{t[r.option_id]=(t[r.option_id]||0)+1});
    csvDownload(`${pollId}-tally.csv`, [["Design","Votes"],["A",t.A],["B",t.B],["C",t.C]]);
  }
  async function exportVotes(){
    const { data:v } = await supabase.from("votes").select("pin,unit,name,option_id,created_at").eq("poll_id",pollId).order("created_at",{ascending:false});
    csvDownload(`${pollId}-votes.csv`, [["PIN","Unit","Owner","Option","Time"], ...(v||[]).map(r=>[r.pin,r.unit||"",r.name||"",r.option_id,new Date(r.created_at).toISOString()])]);
  }
  async function exportPins(){
    const { data:p } = await supabase.from("pins").select("unit,owner_name,pin,token,used_at,used_option").eq("poll_id",pollId).order("unit");
    csvDownload(`${pollId}-pins.csv`, [["Unit","Owner","PIN","Link","Used At","Used Option"], ...(p||[]).map(r=>[r.unit||"",r.owner_name||"",r.pin,`${siteUrl}/?t=${r.token}`,r.used_at||"",r.used_option||""])]);
  }
  ...
  async function generatePins(){
    ...
    const { data: existingRows } = await supabase.from("pins").select("pin").eq("poll_id", pollId);
    ...
    while (toInsert.length < target){
      ...
      existing.add(p);
      toInsert.push({ poll_id: pollId, pin: p });
    }
