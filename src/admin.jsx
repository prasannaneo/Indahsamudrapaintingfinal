...
export function AdminDashboard() {
  ...
  async function refresh(){
    ...
    const a=await supabase.from("vote_audit").select("unit_id, option_id, voter_name, ip, user_agent, created_at").eq("poll_id",pollId).order("created_at",{ascending:false});
    setAudit(a.data||[]);
  }
  ...
  function exportAuditCSV(){const rows=[["Time","Unit","Option","Voter Name","IP","User Agent"]];(audit||[]).forEach(r=>rows.push([r.created_at,r.unit_id,r.option_id,r.voter_name||"",r.ip||"",r.user_agent||""]));downloadCSV(`${pollId}-audit.csv`,rows);}
  ...
  return (<div className="container">
    ...
    <div className="panel">
      <h4>Recent Votes (Audit)</h4>
      <div style={{maxHeight:300,overflow:'auto',border:'1px solid #e5e7eb',borderRadius:8}}>
        <table className="table">
          <thead><tr><th>Time</th><th>Unit</th><th>Option</th><th>Voter</th><th>IP</th><th>User Agent</th></tr></thead>
          <tbody>
            {(audit||[]).map((r,i)=>(
              <tr key={i}>
                ...
                <td>{r.voter_name||"-"}</td>
                <td>{r.ip||"-"}</td>
                <td title={r.user_agent} style={{maxWidth:260,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.user_agent||"-"}</td>
              </tr>
            ))}
          </tbody>
        </table>
