...
function PublicVote(){
  ...
  async function refreshResults(){
    ...
    if(error){ console.error("votes select error", error); }
  ...
  async function lookupPin(){
    ...
    if(error){ alert("Error reading PIN: " + error.message); return; }
  ...
  async function confirmVote(){
    ...
    const {data:fresh,error:chkErr}=await supabase.from("pins").select("used_at").eq("poll_id",pollId).eq("pin",pinRecord.pin).maybeSingle();
    if(chkErr){ alert("Error checking PIN: " + chkErr.message); return; }
    ...
    const {error:insErr}=await supabase.from("votes").insert({ poll_id:pollId, pin:pinRecord.pin, unit:finalUnit, name:finalName, option_id:confirmId });
    if(insErr){ alert("Vote failed: " + insErr.message); setConfirmId(null); return; }

    const { error:updErr } = await supabase.from("pins").update({ used_at:new Date().toISOString(), used_option:confirmId, unit: finalUnit, owner_name: finalName }).eq("poll_id",pollId).eq("pin",pinRecord.pin);
    if(updErr){ alert("Saved vote, but failed to mark PIN used: " + updErr.message); }
...
function AdminDashboard(){
  ...
  async function resetAll(){
    ...
    const { data: delVotes, error: delErr } = await supabase.from("votes").delete().eq("poll_id",pollId).select("id");
    if(delErr){ alert("Failed to delete votes: " + delErr.message); return; }
    const { data: updPins, error: updErr } = await supabase.from("pins").update({ used_at:null, used_option:null }).eq("poll_id",pollId).select("id");
    if(updErr){ alert("Failed to reset pins: " + updErr.message); return; }
    await refresh();
    alert(`Reset done. Cleared ${delVotes?.length||0} vote(s) & marked ${updPins?.length||0} pin(s) unused.`);
  }
