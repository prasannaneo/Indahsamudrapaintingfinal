...
function PublicVote({ hasTokenQS }) {
  ...
  async function confirmVote() {
    ...
    // Log IP + time on server (best effort)
    try {
      await fetch("/api/log-vote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ pollId, unit_id: record.unit_id, option_id: confirmId })
      });
    } catch (_) {}
  ...
  return (
    <div className="container gradient">
      ...
      <style>{`
        ...
        th,td{border-top:1px solid #e5e7eb;padding:6px 8px;text-align:left;font-size:12px}
...
function AdminDashboard() {
  ...
  async function refresh() {
    ...
    const { data: a } = await supabase
      .from("vote_audit")
      .select("created_at, unit_id, option_id, ip, user_agent")
      .eq("poll_id", pollId)
  ...
  async function resetPoll() {
    const input = prompt(
      "Type the setup/reset code to confirm reset.\n\nTip: add !all after the code to clear ALL polls (hard reset)."
    );
    if (!input) return;

    const all = /\!all\b/i.test(input);
    const code = input.replace(/\!all\b/gi, "").trim();

    try { setBusy(true); setMsg(all ? "Hard reset (all polls)..." : "Resetting current poll...");
      const res = await fetch("/api/reset-poll", { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ pollId, code, hard: all }) });
      const out = await res.json().catch(()=>({}));
      if (!res.ok) throw new Error(out?.error || out?.message || "Reset failed");
      setMsg(`Reset complete (${out.scope}).`); await refresh();
  ...
  // -- residents upload (same as earlier build) --
  ...
  function fromRows(fields, data) {
    ...
    for (const r of data) {
      ...
      if (unit && owner) rows.push({ unit_id: unit, owner_name: owner });
    }
    return rows;
  }
  ...
  async function handleUpload(e) {
    ...
    try { setBusy(true); setMsg("Parsing file…");
      ...
      if (!rows.length) { setRowsPreview([]); setMsg("No valid rows found. Check headers: unit_id, owner_name."); setBusy(false); return; }
  ...
  function exportAuditCSV() {
    const rows = [
      ["Time (local)", "Unit", "Option", "IP", "User Agent"],
      ...audit.map(a => [
        new Date(a.created_at).toLocaleString(),
        a.unit_id, a.option_id, a.ip || "", (a.user_agent || "").replaceAll('"','""')
      ])
    ];
    ...
    aTag.href = url;
    aTag.download = `${pollId}-vote-audit.csv`;
    aTag.click();
    ...
    <div className="container">
      ...
      <div className="panel">
        ...
        {rowsPreview.length > 0 && (
          <div style={{ maxHeight: 220, overflow: "auto", marginTop: 8 }}>
            <table>
              <thead><tr><th>unit_id</th><th>owner_name</th></tr></thead>
              <tbody>{rowsPreview.map((r, i) => (<tr key={i}><td>{r.unit_id}</td><td>{r.owner_name}</td></tr>))}</tbody>
            </table>
          </div>
        )}
      </div>
      ...
      <div className="panel">
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontWeight: 600, marginBottom: 6 }}>Vote Audit (latest 200)</div>
          <button className="btn" onClick={exportAuditCSV} disabled={audit.length === 0}>Download CSV</button>
        </div>
        <div style={{ maxHeight: 280, overflow: "auto", marginTop: 6 }}>
          <table>
            <thead>
              <tr>
                <th>Time</th><th>Unit</th><th>Option</th><th>IP</th><th>User Agent</th>
              </tr>
            </thead>
            <tbody>
              {audit.map((a,i)=>(
                <tr key={i}>
                  ...
                  <td>{a.option_id}</td>
                  <td>{a.ip || ""}</td>
                  <td style={{maxWidth: 420, overflow:"hidden", textOverflow:"ellipsis"}} title={a.user_agent || ""}>{a.user_agent || ""}</td>
                </tr>
              ))}
            </tbody>
          </table>
