...
function AdminDashboard(){
  ...
  function templateCSV(){
    ...
    const BOM='\\ufeff'; const csv=rows.map(r=>r.join(",")).join("\\r\\n");
    const blob=new Blob([BOM+csv],{type:"text/csv;charset=utf-8;"});
    const url=URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download="residents_template.csv"; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }
  async function parseCSV(file){
    setRowsPreview([]); setUploadCount(0);
    const text = await file.text();
    const lines = text.split(/\\r?\\n/).filter(Boolean);
    if(lines.length<2){ setMsg("CSV seems empty."); return []; }
    const header = lines[0].split(",").map(s=>s.trim().toLowerCase());
    const idx = { unit_id: header.indexOf("unit_id"), owner_name: header.indexOf("owner_name"), email: header.indexOf("email"), whatsapp: header.indexOf("whatsapp") };
    if(idx.unit_id<0 || idx.owner_name<0){ setMsg("CSV must have headers: unit_id, owner_name, [email], [whatsapp]"); return []; }
    const rows = [];
    for(let i=1;i<lines.length;i++){
      const cols = lines[i].split(",");
      const row = { unit_id:(cols[idx.unit_id]||"").trim(), owner_name:(cols[idx.owner_name]||"").trim(), email: idx.email>=0 ? (cols[idx.email]||"").trim() : "", whatsapp: idx.whatsapp>=0 ? (cols[idx.whatsapp]||"").trim() : "" };
      if(row.unit_id && row.owner_name) rows.push(row);
    }
    setRowsPreview(rows.slice(0,10)); setUploadCount(rows.length); return rows;
  }
  ...
  async function handleUpload(e){
    ...
    try{
      setBusy(true); setMsg("Parsing CSV…");
      const rows = await parseCSV(f); if(!rows.length){ setBusy(false); return; }
