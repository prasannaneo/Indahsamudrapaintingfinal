...
function PublicVote() {
  ...
  async function lookupToken(raw) {
    ...
    try {
      ...
      if (data.expires_at && new Date(data.expires_at) < new Date()) {
        setError("This QR link has expired. Please request a new one.");
      }
      setRecord(data);
    } catch (e) {
  ...
  async function confirmVote() {
    ...
    const { error: insErr } = await supabase.from("votes").insert({
      ...
      unit: record.unit_id, // legacy column safety
      option_id: confirmId,
      voter_name: record.owner_name || null,
      name: record.owner_name || null, // legacy column safety
    });
    if (insErr) {
      alert(
  ...
  return (
    <div className="container">
                ...
                return (
                  <div key={d.id} className="card">
                    ...
                    <img
                      ...
                      onError={(e) => {
                        e.currentTarget.src =
                          "https://placehold.co/800x500?text=Upload+design-" + d.id;
                      }}
                    />
                    <div className="card-body">
                      <div className="row" style={{ justifyContent: "space-between" }}>
                        ...
                        <button className="btn" onClick={() => window.open(d.image, "_blank")}>
                          Preview
                        </button>
                      </div>
      ...
      <style>{`
        ...
        .badge{position:absolute;top:10px;left:10px;background:rgba(0,0,0,.75);color:#fff;border-radius:999px;padding:4px 10px;font-size:12px;font-weight:600}
      `}</style>
    </div>
  );
}
...
function AdminDashboard() {
  ...
  async function refresh() {
    ...
    (v || []).forEach((r) => { tally[r.option_id] = (tally[r.option_id] || 0) + 1; });
    setCounts(tally);
  ...
  async function qrPack() {
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = `${pollId}-QR-pack.pdf`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  ...
  async function sendEmailQRCodes() {
    try { setBusy(true); setMsg("Emailing QR codes…"); const out = await callApiJSON("/api/send-qr-email", {}); setMsg(`Emailed ${out.sent} / ${out.total} residents.`); await refresh(); }
    catch (e) { setMsg("Email QR failed: " + (e.message || e)); }
    finally { setBusy(false); }
  }
  async function sendWhatsAppQRCodes() {
    try { setBusy(true); setMsg("Sending WhatsApp QR codes…"); const out = await callApiJSON("/api/send-qr-whatsapp", {}); setMsg(`WhatsApp sent to ${out.sent} / ${out.total} numbers.`); await refresh(); }
    catch (e) { setMsg("WhatsApp QR failed: " + (e.message || e)); }
    finally { setBusy(false); }
  }
  ...
  async function resetPoll() {
    ...
    if (!code) return;
    try {
      setBusy(true); setMsg("Resetting poll…");
      ...
      setMsg(`Reset complete. Deleted ${out.votesDeleted} vote(s) and ${out.tokensDeleted} token(s).`);
      await refresh();
    } catch(e) { setMsg("Reset failed: " + (e.message || e)); }
    finally { setBusy(false); }
  }
  async function resetResidents() {
    const code = prompt("Type the setup/reset code to delete ALL residents (this also removes all tokens & votes due to cascade):");
    if (!code) return;
    try {
      setBusy(true); setMsg("Resetting resident roster…");
  ...
  // ===== Robust file parsing (CSV or Excel) =====
  ...
  function mapHeaders(fields) {
    ...
    return {
      unit_id: find("unit_id","unit","unit_no","unit_number","unitnumber","unit-no"),
    ...
    if (name.endsWith(".xlsx") || name.endsWith(".xls")) {
      ...
      return await new Promise((resolve) => {
        Papa.parse(text, {
          ...
          complete: (res) => {
            if (!res.data || !res.data.length) return resolve([]);
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await res.text() || res.statusText);
      const out = await res.json(); setMsg(`Upserted ${out.upserted} resident(s).`);
      await refresh();
  ...
  return (
    <div className="container">
      <div className="nav">
        ...
        <span style={{ marginLeft: 8 }}>Logged in as <b>{me}</b></span>
      </div>
      ...
      <div className="panel">
        ...
        {rowsPreview.length > 0 && (
          <div style={{ maxHeight: 220, overflow: "auto", marginTop: 8 }}>
            <table>
              <thead>
                <tr><th>unit_id</th><th>owner_name</th><th>email</th><th>whatsapp</th></tr>
              </thead>
              <tbody>
                {rowsPreview.map((r, i) => (
                  <tr key={i}><td>{r.unit_id}</td><td>{r.owner_name}</td><td>{r.email}</td><td>{r.whatsapp}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
