...
function AdminDashboard(){
  ...
  function templateCSV(){
    const rows=[["unit_id","owner_name","email","whatsapp"],["A-01-01","Tan Wei Ling","weiling.tan@example.com","+6591234567"]];
    const BOM='\ufeff';
    const csv=rows.map(r=>r.join(",")).join("\r\n");
    const blob=new Blob([BOM+csv],{type:"text/csv;charset=utf-8;"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download="residents_template.csv"; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }

  async function parseCSV(file){
    setRowsPreview([]); setUploadCount(0); setUploadMsg("");
    const text = await file.text();
    // lightweight CSV parser (assumes no embedded commas in fields). For complex files, switch to PapaParse.
    const lines = text.split(/\r?\n/).filter(Boolean);
    if(lines.length<2){ setUploadMsg("CSV seems empty."); return; }
    ...
    if(idx.unit_id<0 || idx.owner_name<0){ setUploadMsg("CSV must have headers: unit_id, owner_name, [email], [whatsapp]"); return; }
    const rows = [];
    for(let i=1;i<lines.length;i++){
      const cols = lines[i].split(",");
      const row = {
        unit_id: (cols[idx.unit_id]||"").trim(),
        owner_name: (cols[idx.owner_name]||"").trim(),
        email: idx.email>=0 ? (cols[idx.email]||"").trim() : "",
        whatsapp: idx.whatsapp>=0 ? (cols[idx.whatsapp]||"").trim() : ""
      };
      if(row.unit_id && row.owner_name) rows.push(row);
    }
  ...
  async function uploadResidents(file){
    try{
      setBusy(true); setMsg("Uploading residents…");
      const rows = await parseCSV(file);
      if(!rows || !rows.length){ setMsg("No valid rows found."); setBusy(false); return; }
  ...
  async function sendEmailQRCodes(){
    try{
      setBusy(true); setMsg("Emailing QR codes to residents…");
  ...
  async function sendWhatsAppQRCodes(){
    try{
      setBusy(true); setMsg("Sending WhatsApp QR codes…");
...
<div className="space"></div>
<div className="panel">
  <div style={{fontWeight:600, marginBottom:6}}>Residents Upload (CSV)</div>
  <div className="row" style={{gap:8, alignItems:'center', flexWrap:'wrap'}}>
    <input type="file" accept=".csv" onChange={(e)=>e.target.files&&uploadResidents(e.target.files[0])} disabled={busy} />
    <button className="btn" onClick={templateCSV} disabled={busy}>Download CSV Template</button>
    {uploadCount>0 && <span className="notice">Parsed {uploadCount} row(s). Preview below.</span>}
  </div>
  {rowsPreview.length>0 && (
    <div style={{maxHeight:220, overflow:'auto', marginTop:8}}>
      ...
      <div className="space"></div>
      <div className="notice">All methods issue fresh, single-use links and expire prior unused ones. WhatsApp sending requires E.164 numbers and Twilio WhatsApp credentials.</div>
    </div>
  </div>;
}
