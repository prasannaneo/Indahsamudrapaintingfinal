...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const r of (residents||[])) {
      total++; if (!r.whatsapp || votedSet.has(r.unit_id)) continue;
      ...
      if (!resp.ok) { const err=await resp.text(); throw new Error(`Meta send failed for ${r.unit_id}: ${err}`); }
      await sb.from("vote_tokens").update({ sent_at: new Date().toISOString() }).eq("id", ins.data.id);
      sent++;
    }
    res.status(200).json({ total, sent });
  } catch (e) { console.error(e); res.status(500).send(e.message || "WhatsApp (Meta) send failed"); }
}
