...
import { getSupabaseServer, ensureBucket, assertPublicSiteUrl, getTwilio, getEnv } from "./_config.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: residents, error: rerr } = await sb.from("residents").select("unit_id, owner_name, whatsapp");
    if (rerr) throw rerr;
    ...
    for (const r of residents) {
      ...
      await client.messages.create({
        from, to,
        body: `Hi ${r.owner_name}, scan this QR to vote (one-time, expires in 30 days). If you can't scan, open: ${link}`,
        mediaUrl: [pub]
      });
