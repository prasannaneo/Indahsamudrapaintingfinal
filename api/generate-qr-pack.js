...
const randomToken=(len=12)=>{const abc="ABCDEFGHJKLMNPQRSTUVWXYZ23456789abcdefghijkmnpqrstuvwxyz";return Array.from(crypto.getRandomValues(new Uint8Array(len))).map(b=>abc[b%abc.length]).join('');};

export default async function handler(req,res){
  try{
    const { pollId, validityDays } = req.body||{};
    const site = process.env.PUBLIC_SITE_URL || (req.headers["x-forwarded-proto"]&&req.headers.host?`${req.headers["x-forwarded-proto"]}://${req.headers.host}`:"");
    ...
    for(const r of residents){
      ...
      await db.from("vote_tokens").upsert({poll_id:pollId,unit_id:r.unit_id,owner_name:r.owner_name,token_hash,used_at:null,expires_at},{onConflict:"poll_id,unit_id"});
