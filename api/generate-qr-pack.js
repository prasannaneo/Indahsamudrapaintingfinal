...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let i=0; i<targets.length; i++) {
      ...
      if (i>0) doc.addPage();
      doc.fontSize(18).text("Indah Samudra – Official Voting QR");
      doc.moveDown(0.5);
      doc.fontSize(12).text(`Unit: ${r.unit_id}`); doc.text(`Owner: ${r.owner_name}`);
      doc.moveDown(0.5); doc.image(png, { width: 260 });
      doc.moveDown(0.5); doc.fontSize(11).text("Scan this QR with your phone camera to vote.", { width: 400 });
    ...
    doc.end();
  } catch (e) { console.error(e); res.status(500).send(e.message || "QR pack generation failed"); }
}
