...
export default async function handler(req,res){
  try{
    ...
    for(const t of tokens){
      if(i%per===0){ if(i) doc.addPage(); doc.fontSize(16).text("Indah Samudra Condo Painting – QR Pack"); doc.moveDown(0.2); doc.fontSize(10).fillColor("#334155").text(`Poll: ${pollId} • Generated: ${new Date().toLocaleString()} • Valid: 14 days`); doc.fillColor("black"); doc.moveDown(0.5); }
