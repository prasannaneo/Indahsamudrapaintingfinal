...
export default async function handler(req, res) {
  try {
    ...
    // Build map of current tokens for this poll
    const { data: existing, error: terr } = await supabase
      .from("vote_tokens").select("id, unit_id, token_hash").eq("poll_id", pollId);
    if (terr) throw terr;
    ...
    for (const t of tokens) {
      ...
      doc.fontSize(9).fillColor("#475569").text(t.url, x, y + 260, { width: 220 });
      doc.fillColor("black");
      i++;
    }
