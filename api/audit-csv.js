...
async function fetchAudit(supabase, pollId, includeName){
  const cols=['created_at','unit_id','option_id']; if(includeName) cols.push('voter_name'); cols.push('ip','user_agent');
...
export default async function handler(req,res){
  try{
    ...
    const body=(data||[]).map(r=> (hasName? [r.created_at,r.unit_id,r.option_id,r.voter_name,r.ip,r.user_agent] : [r.created_at,r.unit_id,r.option_id,r.ip,r.user_agent]).map(csvEscape).join(',')).join('\r\n')
    const csv='\uFEFF'+header.join(',')+'\r\n'+body; res.setHeader('Content-Type','text/csv; charset=utf-8'); res.setHeader('Content-Disposition',`attachment; filename="audit-${pollId}.csv"`); res.status(200).send(csv)
