...
export default async function handler(req, res) {
  ...
  try {
    ...
    const site = process.env.PUBLIC_SITE_URL; if (!site) throw new Error("PUBLIC_SITE_URL not set");
    const resendKey = process.env.RESEND_API_KEY; const resendFrom = process.env.RESEND_FROM;
    if (!resendKey || !resendFrom) throw new Error("Resend envs missing");
    const { Resend } = await import("resend"); const resend = new Resend(resendKey);
    ...
    for (const r of (residents||[])) {
      total++; if (!r.email || votedSet.has(r.unit_id)) continue;
      ...
      await resend.emails.send({ from: resendFrom, to: r.email, subject: "Indah Samudra – Your QR to Vote",
  ...
  } catch (e) { console.error(e); res.status(500).send(e.message || "Email QR failed"); }
}
